...
(function(){
  ...
  function estimatedSteps(distanceKm,heightCm,type){
    if(type==='cycle'||!distanceKm) return 0;
    const h=Math.max(120,Math.min(220,Number(heightCm||170)));
    const strideCm=h*(type==='run'?0.65:0.415);
    return Math.max(0,Math.round((distanceKm*100000)/strideCm));
  }
  ...
  function chooser(){
    ...
    <section class="grid grid-3">
      <article class="card"><div class="card-title">Walk</div><div class="card-subtitle">Distance · pace · estimated steps · calories</div><button class="primary-btn full-btn" style="margin-top:14px" data-activity-start="walk">Start Walk</button></article>
      <article class="card"><div class="card-title">Run</div><div class="card-subtitle">Distance · pace · estimated steps · calories</div><button class="primary-btn full-btn" style="margin-top:14px" data-activity-start="run">Start Run</button></article>
  ...
  function renderLive(){
    ...
    <section class="grid grid-2">
      ...
      ${!isCycle?`<article class="card"><div class="metric">${m.steps.toLocaleString()}</div><div class="metric-label">estimated steps</div></article>`:''}
    </section>
    ...
    <div class="grid grid-2" style="margin-top:14px"><button class="secondary-btn" data-activity-pause>${activity.status==='paused'?'Resume':'Pause'}</button><button class="primary-btn" data-activity-finish>Finish</button></div>`;
  }

  function onPosition(pos){if(!activity||activity.status!=='active')return;const c=pos.coords;activity.lastAccuracy=c.accuracy||null;if(c.accuracy&&c.accuracy>70){renderLive();return;}const p={lat:c.latitude,lon:c.longitude,ts:pos.timestamp,accuracy:c.accuracy};const prev=activity.points.at(-1);if(prev){const d=hav(prev,p);if(d>0&&d<0.25)activity.distanceKm+=d;}activity.points.push(p);activity.gpsReady=true;renderLive();}
  ...
  async function begin(type){
    ...
    let weightKg=70,heightCm=170;const {data:profile}=await supabase.from('profiles').select('weight_kg,height_cm').eq('id',user.id).maybeSingle();if(profile?.weight_kg)weightKg=Number(profile.weight_kg);if(profile?.height_cm)heightCm=Number(profile.height_cm);
    activity={type,userId:user.id,status:'active',startedAt:new Date().toISOString(),segmentStartedAt:Date.now(),elapsedMs:0,distanceKm:0,points:[],gpsReady:false,lastAccuracy:null,weightKg,heightCm};
  ...
  function openAndBegin(type){state.route='ride';document.querySelectorAll('.nav-item').forEach(b=>b.classList.toggle('active',b.dataset.route==='ride'));window.fitTrackRender?.();setTimeout(()=>begin(type),0);}
