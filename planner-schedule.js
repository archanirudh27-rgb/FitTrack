...
(function () {
    ...
    module.innerHTML = `
      ...
      <div data-fit-upcoming-list style="margin-top:10px">
        ${(planned||[]).length ? planned.map(s=>`
          <div class="meta-row" style="padding:10px 0;border-top:1px solid rgba(255,255,255,.08);gap:10px">
            ...
            <div style="display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end">
              <button type="button" class="secondary-btn" data-fit-session-load="${s.id}">Load to Session</button>
  ...
  async function scheduleWorkout(button) {
    ...
    if (tErr || !template) { button.disabled=false; button.textContent='Schedule workout'; toast?.('Could not load workout'); return; }
    ...
    if (duplicate?.length) { button.disabled=false; button.textContent='Schedule workout'; toast?.('This workout is already scheduled for that date'); return; }
    const { error } = await supabase.from('planned_sessions').insert({ user_id:user.id, template_id:templateId, session_date:date, name:template.name, status:'planned' });
    button.disabled=false; button.textContent='Schedule workout';
    if (error) { toast?.('Could not schedule workout'); return; }
    toast?.('Workout scheduled'); document.querySelector('[data-fit-schedule-module]')?.remove(); enhancePlanner();
  }
  ...
  async function removeScheduled(id) {
    ...
    toast?.('Scheduled workout removed'); document.querySelector('[data-fit-schedule-module]')?.remove(); enhancePlanner();
  }

  document.addEventListener('click', event => {
    const save = event.target.closest('[data-fit-schedule-save]'); if (save) { event.preventDefault(); scheduleWorkout(save); return; }
    const del = event.target.closest('[data-fit-schedule-delete]'); if (del) { event.preventDefault(); removeScheduled(del.dataset.fitScheduleDelete); return; }
  });
