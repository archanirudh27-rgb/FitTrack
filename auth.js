...
(function () {
  ...
  async function applySession(session, allowOnboarding=false){
    ...
    profileButton.textContent=initials(displayName);
  ...
  function showOnboarding(){
    document.getElementById('fittrackOnboarding')?.remove();
  ...
  function showAccount(){
    ...
    const panel=document.createElement('div');panel.id='fittrackAccount';panel.className='account-panel';
    panel.innerHTML=`<strong>${u.display_name||'FitTrack User'}</strong><div class="account-email">${u.email||''}</div><div class="account-meta"><div><span>Weight</span><b>${p.weight_kg?`${Number(p.weight_kg).toFixed(1)} kg`:'Not set'}</b></div><div><span>Goal</span><b>${String(m.training_goal||'General').replace('-',' ')}</b></div></div><button class="account-btn" id="fittrackEditProfile" style="margin-bottom:8px">Edit profile</button><button class="account-btn" id="fittrackLogout">Sign out</button>`;
    document.body.appendChild(panel);
    document.getElementById('fittrackEditProfile').onclick=()=>{panel.remove();showOnboarding();};
    document.getElementById('fittrackLogout').onclick=async()=>{await client.auth.signOut();panel.remove();window.fitTrackUser=null;resetPersonalState();setLoggedOutIcon();showAuth('login');};
  }

  profileButton.onclick=showAccount;setLoggedOutIcon();
